...
set(fmt_off "${esc}[m")
set(fmt_ok "${esc}[97m")
set(fmt_location "${esc}[93m")
set(fmt_error "${esc}[91m")
    ...
    if(TOOL STREQUAL "iwyu" OR TOOL STREQUAL "pch")
        foreach(line IN LISTS content)
            ...
            elseif(line MATCHES "^(.+)( should.*)$")
                ...
                message("${fmt_location}${path}${fmt_text}${CMAKE_MATCH_2}${fmt_off}")
            elseif(line MATCHES "^(The full include-list for )(.+)(:)$")
                cmake_path(RELATIVE_PATH CMAKE_MATCH_2 BASE_DIRECTORY "${SOURCE_DIR}" OUTPUT_VARIABLE path)
                message("${fmt_text}${CMAKE_MATCH_1}${fmt_location}${path}${fmt_text}${CMAKE_MATCH_3}${fmt_off}")
            elseif(line MATCHES "^(.+):$")
    ...
    elseif(TOOL STREQUAL "clang-tidy")
        foreach(line IN LISTS content)
            if(line MATCHES "^([^ ].?[^:]*)(:[0-9]+:[0-9]+: )([^:]+)(: .*)(\\[.+\\])$")
                cmake_path(RELATIVE_PATH CMAKE_MATCH_1 BASE_DIRECTORY "${SOURCE_DIR}" OUTPUT_VARIABLE path)
                if(CMAKE_MATCH_3 STREQUAL "error")
                    set(fmt_message "${fmt_error}")
                else()
                    set(fmt_message "${fmt_text}")
                endif()
                message("${fmt_location}${path}${CMAKE_MATCH_2}${fmt_message}${CMAKE_MATCH_3}${CMAKE_MATCH_4}${fmt_rule}${CMAKE_MATCH_5}${fmt_off}")
            else()
                message("${line}")
            endif()
        endforeach()
